 ...
 * Mimics exactly how the staging app calls Gemini
 */
...
async function testGeminiCall() {
  ...
  // Load THREE images - testing if 3 is the critical threshold to eliminate graph dependencies
  const imagePaths = [
  ...
  try {
    ...
    try {
      ...
      // Save to file
      const outputPath = path.join(__dirname, 'test-output-direct.json');
...
// Run the test
testGeminiCall().catch(console.error);
