/**
 * Test Gamified Key Concepts Generation - German History Edition
 * Tests Gemini's ability to generate exam questions, educational summary, AND gamified key concepts in one call
 * Uses German history textbook images to test language detection and humanities content
 *
 * Expected Output:
 * - 15 exam questions (in German)
 * - ~970 word educational summary (4 sections, in German)
 * - 8-12 key concepts with gamification metadata (in German)
...
async function testGamifiedConceptGeneration() {
  ...
  try {
    ...
    try {
      ...
      const outputPath = path.join(__dirname, 'prompttests', `test-gamified-concepts-german-history-${timestamp}.json`);
      ...
      if (parsed.key_concepts && parsed.key_concepts.length > 0) {
        ...
        const htmlPath = path.join(htmlOutputDir, `key-concepts-german-history-${timestamp}.html`);
      ...
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const errorPath = path.join(__dirname, 'prompttests', `test-gamified-concepts-german-history-ERROR-${timestamp}.txt`);
...
// Run the test
console.log('🚀 Starting Gamified Key Concepts Test - German History Edition');
console.log('='.repeat(80));
