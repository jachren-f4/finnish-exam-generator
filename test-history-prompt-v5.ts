/**
 * Test Script for V5 History Prompt (Visual Confirmation Rule)
 *
 * V5 adds Visual Confirmation Rule to prevent outside knowledge:
 * - Only use names, dates, events visible in textbook images
 * - If fact not visible, skip it (even if historically true)
 * - Double-check validation: questions must reference visible text
 */
...
async function generateExpertHistoryExam(imageParts: Array<{ inlineData: { data: string; mimeType: string } }>) {
  console.log(`\n🧪 Testing V5 prompt with Visual Confirmation Rule (Flash-Lite optimized) with ${imageParts.length} images...\n`)
  ...
  try {
    ...
    try {
      ...
      writeFileSync('test-output-history-v5-raw.txt', text)
      throw parseError
    }
    ...
    const output = {
      test_metadata: {
        ...
        version: 'v5-visual-confirmation',
        prompt_source: 'V5 - Visual Confirmation Rule',
        model: 'gemini-2.0-flash-exp',
        temperature: 0,
    ...
    writeFileSync('test-output-history-v5.json', JSON.stringify(output, null, 2))
    console.log(`\n💾 Saved to: test-output-history-v5.json\n`)
...
async function main() {
  console.log('🏛️  V5 HISTORY PROMPT TESTER (Visual Confirmation Rule)')
  console.log('====================================================\n')
