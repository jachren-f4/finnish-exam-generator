/**
 * Test Gamified Key Concepts Generation - Language & Subject Agnostic Version
 * Tests Gemini's ability to generate exam questions, educational summary, AND gamified key concepts in one call
 * WITHOUT specifying language or subject - fully automatic detection
 *
 * Expected Output:
 * - 15 exam questions (in detected language)
 * - ~970 word educational summary (4 sections, in detected language)
 * - 8-12 key concepts with gamification metadata (in detected language)
 * - Gamification completion data
 * - HTML prototype with correct language UI strings
 *
 * Features:
 * - Automatic language detection from source material
 * - Automatic subject detection from content
 * - Multi-language HTML UI generation (Finnish, German, English, etc.)
 * - No hardcoded language or subject assumptions
 */
...
// No hardcoded category or grade - will be detected automatically from content

// Combined prompt: Questions + Summary + Gamified Key Concepts
// Language-agnostic and subject-agnostic version - Gemini auto-detects both
const prompt = `Create a text-based exam from educational content for students AND generate an educational summary AND identify and gamify the key learning concepts.
...
TARGET: Automatically detect and use the SAME language as the source material. Detect the subject area from the content.
...
- Use a friendly, teacher-like tone suitable for students
...
- "category": a short category label (1–2 words) that best represents the group or theme the concept belongs to, appropriate for the subject matter
- "related_question_ids": array of integers referring to the question IDs above
- "badge_title": a short gamified label appropriate for the subject and language
...
Try to create diverse category labels across all key concepts based on the actual subject matter. Avoid assigning the same category to all concepts.
...
REQUIRED FORMAT:
{
  ...
  "key_concepts": [
    {
      "concept_name": "[2-5 word concept name in source language]",
      "definition": "[Clear definition in source language]",
      "difficulty": "intermediate",
      "category": "[Subject-appropriate category]",
      "related_question_ids": [1, 4, 7],
      "badge_title": "[Gamified label in source language]",
      "mini_game_hint": "[Riddle or clue in source language]"
    }
  ],
  "gamification": {
    "completion_message": "[Celebration message in source language]",
    "boss_question_open": "[Open-ended synthesis question in source language]",
    "boss_question_multiple_choice": {
      "question": "[Multiple choice synthesis question in source language]",
      "options": [
        "[Option 1 in source language]",
        "[Option 2 in source language]",
        "[Option 3 in source language]",
        "[Option 4 in source language]"
      ],
      "correct_answer": "[Matching option in source language]"
    },
    "reward_text": "[Reward message in source language]"
  }
}
...
const UI_STRINGS_MAP: Record<string, UIStrings> = {
  en: {
    ...
    completionBanner: "🎉 Congratulations! You've learned all key concepts!",
    headerTitle: "Key Concepts",
    totalConcepts: "Total Concepts",
    progress: "Progress"
  },
  fi: {
    showExplanation: "Näytä selitys",
    hideExplanation: "Piilota selitys",
    definition: "Määritelmä",
    hint: "Vihje",
    conceptsLearned: "Opittuja käsitteitä",
    completionBanner: "🎉 Onneksi olkoon! Olet oppinut kaikki avainkäsitteet!",
    headerTitle: "Avainkäsitteet",
    totalConcepts: "Käsitteitä yhteensä",
  ...
  de: {
    ...
    definition: "Definition",
    hint: "Hinweis",
    conceptsLearned: "Gelernte Konzepte",
    completionBanner: "🎉 Herzlichen Glückwunsch! Du hast alle Schlüsselkonzepte gelernt!",
    headerTitle: "Schlüsselkonzepte",
    totalConcepts: "Konzepte insgesamt",
  ...
  sv: {
    ...
    definition: "Definition",
    hint: "Ledtråd",
    conceptsLearned: "Lärda koncept",
    completionBanner: "🎉 Grattis! Du har lärt dig alla nyckelkoncept!",
    headerTitle: "Nyckelkoncept",
    totalConcepts: "Totalt koncept",
    progress: "Framsteg"
  },
  es: {
    ...
    hint: "Pista",
    conceptsLearned: "Conceptos aprendidos",
    completionBanner: "🎉 ¡Felicitaciones! ¡Has aprendido todos los conceptos clave!",
    headerTitle: "Conceptos clave",
    totalConcepts: "Conceptos totales",
    progress: "Progreso"
  },
  fr: {
    ...
    hint: "Indice",
    conceptsLearned: "Concepts appris",
    completionBanner: "🎉 Félicitations! Vous avez appris tous les concepts clés!",
    headerTitle: "Concepts clés",
    totalConcepts: "Total des concepts",
    progress: "Progrès"
  }
};
 ...
 */
function getUIStrings(languageCode: string): UIStrings {
  // Try exact match first
  if (UI_STRINGS_MAP[languageCode]) {
    return UI_STRINGS_MAP[languageCode];
  }

  // Try lowercase
  const lowerCode = languageCode.toLowerCase();
  if (UI_STRINGS_MAP[lowerCode]) {
    return UI_STRINGS_MAP[lowerCode];
  }

  // Fallback to English
  console.log(`⚠️  Language "${languageCode}" not found in UI strings, falling back to English`);
  return UI_STRINGS_MAP.en;
}
 ...
 * Language-agnostic version - uses detected language for UI strings
 */
function generateConceptHTML(concepts: any[], testConfig: any, languageCode: string): string {
  // Get localized UI strings for detected language
  ...
  const cards = concepts.map((concept, idx) => `
    <div class="concept-card" data-clicked="false" data-index="${idx}">
      ...
      <button class="expand-btn" onclick="toggleDefinition(this)">${ui.showExplanation}</button>
      <div class="definition hidden">
        <p class="definition-text"><strong>${ui.definition}:</strong> ${concept.definition}</p>
        <p class="hint">💡 <strong>${ui.hint}:</strong> ${concept.mini_game_hint}</p>
      </div>
    </div>
  `).join('');
...
<body>
  <div class="container">
    ...
    <div class="stats-bar">
      <div class="stat">
        ...
        <div class="stat-label">${ui.conceptsLearned}</div>
      </div>
      <div class="stat">
        <div class="stat-value">${concepts.length}</div>
        <div class="stat-label">${ui.totalConcepts}</div>
      </div>
      <div class="stat">
        <div class="stat-value" id="progress-percent">0%</div>
        <div class="stat-label">${ui.progress}</div>
      </div>
    </div>

    <section id="key-concepts">
      <h2>${ui.headerTitle}</h2>
      <p id="progress" class="progress" data-ui-show="${ui.showExplanation}" data-ui-hide="${ui.hideExplanation}">0/${concepts.length}</p>
  ...
  <script>
    function toggleDefinition(btn) {
      ...
      // Get UI strings from data attributes on progress element
      const progressEl = document.getElementById('progress');
      btn.textContent = wasHidden ? progressEl.dataset.uiHide : progressEl.dataset.uiShow;
    }
    ...
    function updateProgress() {
      ...
      // Update progress text (simple numeric format - language-agnostic)
      document.getElementById('progress').textContent = mastered.length + '/' + all.length;
...
async function testGamifiedConceptGeneration() {
  ...
  // Load test images (using German history images from assets)
  const imagesDir = path.join(__dirname, 'assets/images/history_de');
  const imageFiles = [
    'sivu46.jpeg',
    'sivu48.jpeg',
    'sivu50.jpeg',
  ...
  console.log(`   Mode: Language & Subject Agnostic (auto-detection)`);
  ...
  try {
    ...
    try {
      ...
      const outputPath = path.join(__dirname, 'prompttests', `test-agnostic-concepts-${timestamp}.json`);
      ...
      fs.writeFileSync(outputPath, JSON.stringify({
        test_config: {
          mode: 'language_agnostic',
          detected_language: parsed.summary?.language || 'unknown',
      ...
      if (parsed.key_concepts && parsed.key_concepts.length > 0) {
        ...
        // Extract detected language for UI strings
        const detectedLanguage = parsed.summary?.language || 'en';
        ...
        const htmlPath = path.join(htmlOutputDir, `key-concepts-agnostic-${timestamp}.html`);
      ...
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const errorPath = path.join(__dirname, 'prompttests', `test-agnostic-concepts-ERROR-${timestamp}.txt`);
